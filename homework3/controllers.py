...
def view_products():
    ...
    sortingstate= request.params.get('sort', 'none')
    rows= db(db.product).select()
    
    if sortingstate == 'asc':
        rows = db(db.product).select(orderby=db.product.product_cost)
    elif sortingstate == 'desc':
        rows = db(db.product).select(orderby=~db.product.product_cost)
    else:
        rows= db(db.product).select()
        
    return dict(rows=rows, url_signer= url_signer, sortingstate=sortingstate)
...
@action.uses('index.html', url_signer.verify(), session, db)
def delete_product(product_id = None):
    row= db(db.product.id== product_id)
    row.delete()
    redirect(URL('index'))
